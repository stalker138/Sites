...
$pb_domain='http://www.elakor-pol.ru';	// Имя домена. Используется при формировании sitemap.xml
...
// Страницы колонок page1_, page2_, page3_ всегда считаются php-скриптами страниц и могут быть включены в sitemap.xml с помощью флажков «sitemap» в окне информации админки.
...
$pb_path_main=$pb_path_root.'apb_pr/';				// Корневая папка Asw pages builder (APB)
...
// Следующие наименования вложенных директорий МЕНЯТЬ НЕЛЬЗЯ : config/, pages/, templates/, blocks/, texts/ - ЭТИ ПРАВИЛО И ХАРДКОД в php-файлах адимнки!!!
...
$pb_path_images=$pb_path_main.'img/';					// МЕНЯТЬ НЕЛЬЗЯ! Папка с файлами изображений для работы APB
$pb_path_files=$pb_path_main.'files/';					// МЕНЯТЬ НЕЛЬЗЯ! Папка с различными файлами для работы APB
...
$pb_attr_folders=0755;		// Права доступа к создаваемым папкам (восьмеричное число!)
$pb_attr_files=0644;		// Права доступа к создаваемым php-файлам (восьмеричное число!)
...
/*
	...
	Функция создает вложенные директории и файлы для работы APB - если они не существуют и была успешная авторизация.
*/
function fpb_authorization($flag) {
	global $pba_login,$pba_pass,$pba_name,$pb_hash,$pb_path_config,$pb_path_pages,$pb_path_templates,$pb_path_blocks,$pb_path_texts,$pb_path_images,$pb_path_files,$pb_attr_folders,$pb_attr_files,$pb_path_sources,$pb_path_backup,$pba_files,$pb_path_site,$pb_path_main,$pb_path_root;
	...
	if ($pb_name!=false && $flag==0) {	// Проверка и создание вложенных папок и файлов для работы
		...
		// $txt=substr($pb_path_images,0,strlen($pb_path_images)-1); if (is_dir($txt)==false) { @mkdir($txt,$pb_attr_folders); @chmod($txt,$pb_attr_folders); }	// Папка для изображений
	...
	if ($pb_name==false && $flag==0) {
		...
		$txt.='<title>Asw pages builder</title><meta name="robots" content="noindex, nofollow"></head><body><form id="form1" action="';
		$txt.=$pb_path_site.substr($pb_path_main,strlen($pb_path_root)).'" method="POST" enctype="application/x-www-form-urlencoded">';
		$txt.='<table style="margin:auto;text-align:center;position:relative;top:200px;letter-spacing:1px;"><tr><td colspan="2" style="text-align:center;color:#800000;">Asw pages builder 00.77</td></tr>';
