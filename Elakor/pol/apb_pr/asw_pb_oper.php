...
include "config.php"; fpb_authorization(5);
...
for ($j=0; $j<6; $j++) {
  $txt=@file_get_contents($pb_path_config.$pba_files[$j].'.php'); if($txt==false) continue; $pos0=0;
  while (true) {
	...
	$pos0=$pos+9; if ($j==0 && ($pb_sitemap_start==0 || $pb_sitemap_start>=$n)) continue; // Если php-скрипт общего назначения (не только url-страница) - ВСЕГДА в колонке page0_ !!!
	$v_priority='0.'.substr($txt,$pos3+13,1); if ($v_priority=='0.0') continue;
...
if ($cnt==0) { print 'error¡'.$px15.'Нет ни одной страницы для sitemap.xml'; }
	...
	$out='<?xml version="1.0" encoding="UTF-8"?>'.chr(10).'<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.5">'.chr(10).$out.'</urlset>'.chr(10);
