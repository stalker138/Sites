<?php
// (c) Vcorp generator 00.80 beta | 2016-2020 | vcorp.ru

// Генерация исходников (элементов) с авто-проверкой.

include "vpb_config.php"; fpb_authorization(2);

$v_mode=(int)$_POST['mode']; $v_index=(int)$_POST['index']; $v_number=(int)$_POST['number'];
if ($v_mode!=30 || $v_index<0 || $v_index>8 || $v_number<0 || $v_number>999 || ($v_index==0 && $v_number!=0)) { print 'error_authorization_2'; exit; }
...
// Генерация
for ($j=0; $j<8; $j++) {
  if ($v_index>0) $j=$v_index-1; $txt=$tx[$j]; $pos0=0; $v_priority=0;
  for ($i=1; $i<1000; $i++) {
	...
	} $pos1=strpos($txt,'*options*',$pos); $v_priority=(int)substr($txt,$pos1+13,1);
	...
	if (substr($v_link,0,$check_url)==substr($pb_path_main,-$check_url)) { print 'error¡</span>Неправильный URL ( директория '.$spanr.'Vcorp generator</span> )'.$t7; exit; }
	...
	$v_title=trim(substr($txt,$pos2,$pos3-$pos2)); $v_title = (empty($v_title)||$v_title=='empty') ? ' ' : htmlspecialchars($v_title);	$v_title=str_replace('{2}','Москва',$v_title);
	...
	$v_desc=trim(substr($txt,$pos2,$pos3-$pos2)); $v_desc = (empty($v_desc)||$v_desc=='empty') ? ' ' : htmlspecialchars($v_desc);	$v_desc=str_replace('{1}','Опыт 30 лет, более 10000 объектов.',$v_desc);
	...
	$pos2=strpos($t,'{*tpl0*}',0); if ($pos2!=false) { // Если вставлять title,keywords,description необходимо ТОЛЬКО php-файлам, то раскомментировать 1 и 3 строки ниже.
	  // $file=$v_link; if (substr($file,-1)=='/') $file.=$pb_main_file; if (substr($file,-4)=='.php') {
		$t=substr($t,0,$pos2).'<meta http-equiv="Content-Type" content="text/html; charset=UTF-8"><title>'.$v_title.'</title><meta name="description" content="'.$v_desc.'"><meta name="keywords" content="'.$v_keyw.'">'.substr($t,$pos2+8);
	  // }
	...
	while (true) {
		...
		if ($n==0) $pos2+=2; else {
			$repl=@file_get_contents($pb_path_templates.'tpl'.$n); if ($repl==false) $repl='';
			if($v_priority>0 && $n==4) { $repl=chr(10).'<meta name="robots" content="index, follow, noarchive">'.chr(10).$repl; }
			if($v_priority==0 && $n==4) { $repl=chr(10).'<meta name="robots" content="noindex, nofollow, noarchive">'.chr(10).$repl; }
			$t=substr($t,0,$pos2).$repl.substr($t,$pos3+2);
			$pos2+=strlen($repl);
		}
	}
	...
	for ($k=0; $k<10; $k++) {
		$pos2=0; $b='{*block'.$k.'_'; while (true) {
			$pos2=strpos($t,$b,$pos2); if ($pos2===false) break; if ($k>7) { print $t4.substr($t,$pos2,10).$t2; exit; }
	...
	$pos=0; while (true) {
		...
		if ($pb_search_url) { // Проверка существующих внутренних URL
			...
			if ($link!=$pb_empty_url && (substr($link,0,1)!='#') && (substr($link,0,10)!='javascript') && (substr($link,0,7)!='mailto:') && (substr($link,0,6)!='skype:') && (substr($link,0,4)!='tel:') && (substr($link,0,7)!='http://') && (substr($link,0,8)!='https://')) for ($k=0; $k<8; $k++) { $pos1=strpos($tx[$k],('*link*'.$t3.$pos3.chr(10)),0); if($pos1>0) break; }
	...
	if ($v_mode==30) {
		...
		if (substr($file,-4)=='.php') { // Если php-файл, то первой строкой генерируются RunTime - переменные:
		  $repl='<?php $pb_domain="'.$pb_domain.'"; '; // prorocol + domain
		  $repl.='$pb_root="'.$pb_path_root.'"; $pb_start_site="'.$pb_path_site.'"; '; // Текущая папка корня веб-ресурса (например для возможности делать include независимо от реального местоположения скриптов).
		  if ($j==0 && ($pb_sitemap_start==0 || $pb_sitemap_start>$i)) { // Если php-скрипт общего назначения (не только url-страница). ВСЕГДА в колонке page0_ !!!
			$repl.='$pb_php'.$i.'=true; $pb_special='.$i.';'; // Наличие переменной $pb_phpN (где N - номер в колонке page0_) говорит о включении этого php-скрипта в конечный php-скрипт. $pb_special - номер строки в колонке 0 для спец.скрипта.
		  } else { // Остальные php-скрипты (url - страницы).
			$repl.='$pb_special=0; $pb_page="'.($j*1000+$i).'"; $pb_page_index='.$j.'; $pb_page_number='.$i.'; $pb_short_url="'.$v_link.'"; $pb_full_url="'.str_replace('//','/',($pb_path_site.$v_link)).'"; $pb_full_path="'.str_replace('//','/',($pb_path_root.$v_link)).'";';
		  } $t=$repl.' ?>'.chr(10).$t;
		}
