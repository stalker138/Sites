...
<link rel="stylesheet" href="vpb1.css" type="text/css"/><script type="text/javascript" charset="UTF-8" src="vpb1.js"></script></head><body><div id="main">
<?php
...
$a1=array('0','1','2','3','4','5','6','7','tpl','0','1','2','3','4','5','6','7','0','1','2','3','4','5');
$out.='</div><div id="tarea">';
for ($i=0; $i<23; $i++) {
	if ($i<8) $tmp=' style="width:30px;color:#000080;border:solid 1px #000080;'.(($i==0)?'margin-left:0px;background-color:#c8c8c8':'').'"'; else $tmp=($i==8||$i==9||$i==17)?(' style="margin-left:15px;'.(($i==8)?'width:40px;color:#005093;border:solid 1px #005093;':(($i==17)?'color:#006000;border:solid 1px #006000;border-radius:5px;':'')).'"'):(($i>17)?' style="color:#006000;border:solid 1px #006000;border-radius:5px;"':'');
...
// Вывод 23-ти блоков области iarea с содержимым
$out.='<div id="iarea">';
for ($j=0; $j<23; $j++) {
  ...
  $tx1='<tr><td style="color:#800000;font:bold 12px arial;" colspan="'.(($j<8)?'8':'').'">error config file '.$pba_files[$j].' number: '; $tx2='</td></tr>'; $f=0;
  for ($i=1;$i<1000; $i++) {
	...
	if ($f==0) $tx.='<tr id="tr'.$j.'_'.$i.'" onclick="f_c('.$j.','.$i.')"><td style="width:'.(($j==8)?'50':'80').'px;">'.$pos1.'</td><td style="width:'.(($j<8)?'20':'25').'px;">'.((substr($pos2,0,1)>'0')?'a':'').'</td>'.(($j<8)?('<td style="width:20px;">'.((substr($pos2,1,1)>'0')?'u':'').'</td><td style="width:35px;">'.$tm.'</td>'):'').'<td id="d3_'.$j.'_'.$i.'">'.$pos3.'</td></tr>';
	else $tx.='<tr id="tr'.$j.'_'.$i.'" onclick="f_c('.$j.','.$i.')"><td>'.$pos1.'</td><td>'.((substr($pos2,0,1)>'0')?'a':'').'</td>'.(($j<8)?('<td>'.((substr($pos2,1,1)>'0')?'u':'').'</td><td>'.$tm.'</td>'):'').'<td id="d3_'.$j.'_'.$i.'">'.$pos3.'</td></tr>';
