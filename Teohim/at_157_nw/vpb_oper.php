...
if ($v_mode<33) {
	if ($v_mode<31 || $v_index<0 || $v_index>23 || $v_number<0 || $v_number>999 || ($v_index==0 && $v_number!=0)) { print 'error_authorization_5'; exit; }
}
...
for ($j=0; $j<23; $j++) {
	...
	$name='page'.$j.'_'; if ($j==8) $name='tpl'; if ($j>8 && $j<17) $name='block'.($j-9).'_'; if ($j>16) $name='text'.($j-17).'_';
  ...
  for ($i=1; $i<1000; $i++) {
	...
	if ($j<8) { $file=$pb_path_pages.$name.$i; $fbfile=$pb_path_backup.'pages/'.$name.$i; }
	if ($j==8) { $file=$pb_path_templates.$name.$i; $fbfile=$pb_path_backup.'templates/'.$name.$i; }
	if ($j>8 && $j<17) { $file=$pb_path_blocks.$name.$i; $fbfile=$pb_path_backup.'blocks/'.$name.$i; }
	if ($j>16) { $file=$pb_path_texts.$name.$i; $fbfile=$pb_path_backup.'texts/'.$name.$i; }
...
if ($v_index>0) {
	$j=$v_index-1;
	if ($j<8) { $t1='страниц'; $t2='page'.$j; }
	if ($j==8) { $t1='шаблонов'; $t2='tpl'; }
	if ($j>8 && $j<17) { $t1='блоков'; $t2='block'.($j-9); }
	if ($j>16) { $t1='текстов'; $t2='text'.($j-17); }
}
if ($v_mode==31) {
	if ($v_index>0 && $v_number==0) print 'ok¡Скопированы все исходники '.$t1.(($j==8)?($spanr.$t2):(' с индексом'.$spanr.$t2.'_')).'</span>';
	if ($v_index>0 && $v_number>0) print 'ok¡Копирование в резерв '.$spanr.$t2.(($j==8)?'':'_').$v_number.'</span> завершено';
	if ($v_index==0 && $v_number==0) print 'ok¡Скопированы исходники всех элементов';
}
if ($v_mode==32) {
  if ($v_index>0 && $v_number==0) print 'ok¡Восстановлены все исходники '.$t1.(($j==8)?($spanr.$t2):(' с индексом'.$spanr.$t2.'_')).'</span>';
  if ($v_index>0 && $v_number>0) print 'ok¡Восстановление из резерва '.$spanr.$t2.(($j==8)?'':'_').$v_number.'</span> завершено';
  if ($v_index==0 && $v_number==0) print 'ok¡Восстановлены исходники всех элементов.'; else {
	$txh=''; $txt=$tx; $tx=''; $tx1='<tr><td style="color:#800000;font:bold 12px arial;" colspan="'.(($j<8)?'4':'').'">error config file '.$pba_files[$j].' number: '; $tx2='</td></tr>'; $f=0;
  for ($i=1;$i<1000; $i++) {
...
if ($cnt==0) { print 'error¡'.$px15.'Нет ни одной страницы для sitemap.xml'; }
	...
	$out='<?xml version="1.0" encoding="UTF-8"?>'.chr(10).'<urlset xmlns="https://www.sitemaps.org/schemas/sitemap/0.9">'.chr(10).$out.'</urlset>'.chr(10);
