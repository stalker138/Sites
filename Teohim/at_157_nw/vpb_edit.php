...
// Дополнительно - поиск правильности внутренних URL - в зависимости от значения $pb_search_url - в файле vpb_config.php

include "vpb_config.php"; fpb_authorization(1);
...
$name='page'.$v_index.'_'; if ($v_index==8) $name='tpl'; if ($v_index>8 && $v_index<17) $name='block'.($v_index-9).'_'; if ($v_index>16) $name='text'.($v_index-17).'_';
$t3=chr(9).chr(9).chr(9); $v_name='empty'.$number; $v_link='empty'; $v_title='empty'; $v_keyw='empty'; $v_desc='empty'; $v_number=0;
$v_options = ($pb_autocheck?'1':'0').(($v_index<8) ? (($pb_titles?'1':'0').($pb_sitemap?(($v_index>0||($pb_sitemap_start>0&&$pb_sitemap_start<=$number))?'5':'0'):'0').'0000000') : '000000000');
...
if ($v_mode==12 || $v_mode==13) { // Добавить || Добавить с копированием элемента $number
	...
	for ($i=1; $i<1000; $i++) {
		...
		if ($v_index<8) { $str.='*link*'.$t3.$v_link.chr(10).'*title*'.$t3.$v_title.chr(10).'*keyw*'.$t3.$v_keyw.chr(10).'*desc*'.$t3.$v_desc.chr(10); }
...
if ($v_mode==21) {
	$v_options=trim(fpb_post('options')); $v_options=($v_index<8) ? $v_options : (substr($v_options,0,1).'000000000'); $v_name=trim(fpb_post('name')); $v_link=trim(fpb_post('link')); $v_title=trim(fpb_post('title')); $v_keyw=trim(fpb_post('keyw')); $v_desc=trim(fpb_post('desc'));
	if ($v_index<8) {
	  ...
	  $pos=strlen(substr($pb_path_main,strlen($pb_path_root)-1)); if (substr($v_link,0,$pos)==substr($pb_path_main,-$pos)) { print 'error¡</span>Неправильный URL'.$px5.'( директория '.$spanr.'Vcorp generator</span> )'.$spanr; exit; }
	  for ($i=0; $i<8; $i++) {
		if ($v_link!='empty') {
		  $str=0; $pos=0;
	...
	if (substr($v_options,0,1)!='0') { // Авто - проверка исходного кода (auto-check)
		...
		if ($v_index>7) f_cl('tpl');
		if ($v_index>8) f_cl('block');
		if ($v_index>16) f_cl('text0');
		if ($v_index>17) f_cl('text1');
		if ($v_index>18) f_cl('text2');
		if ($v_index>19) f_cl('text3');
		if ($v_index>20) f_cl('text4');
		if ($v_index>21) f_cl('text5');
		...
		// Проверка {*block0_N*}, ... , {*block7_N*} на правильность вложения
		for ($k=0; $k<10; $k++) {
			$pos2=0; $b='{*block'.$k.'_'; while (true) {
				$pos2=strpos($t,$b,$pos2); if ($pos2===false) break; if ($k>7) { print $t4.substr($t,$pos2,10); exit; }
		...
		$pos=0; while (true) {
			...
			if ($pb_search_url) { // Проверка существующих внутренних URL
				...
				if ($link!='empty' && (substr($link,0,1)!='#') && (substr($link,0,10)!='javascript') && (substr($link,0,7)!='mailto:') && (substr($link,0,6)!='skype:') && (substr($link,0,4)!='tel:') && (substr($link,0,7)!='http://') && (substr($link,0,8)!='https://')) for ($i=0; $i<8; $i++) { $pos1=strpos($tx[$i],('*link*'.$t3.$pos3.chr(10)),0); if($pos1>0) break; }
...
if ($v_mode==11) {
  ...
  $j=$v_index; $txh=''; $tx='';
  $tx1='<tr><td style="color:#800000;font:bold 12px arial;" colspan="'.(($j<8)?'4':'').'">error config file '.$pba_files[$j].' number: '; $tx2='</td></tr>'; $f=0;
  for ($i=1;$i<1000; $i++) {
	...
	if ($f==0) $tx.='<tr id="tr'.$j.'_'.$i.'" onclick="f_c('.$j.','.$i.')"><td style="width:'.(($j==8)?'50':'80').'px;">'.$pos1.'</td><td style="width:'.(($j<8)?'20':'25').'px;">'.((substr($pos2,0,1)>'0')?'a':'').'</td>'.(($j<8)?('<td style="width:20px;">'.((substr($pos2,1,1)>'0')?'u':'').'</td><td style="width:35px;">'.$tm.'</td>'):'').'<td id="d3_'.$j.'_'.$i.'">'.htmlspecialchars($pos3).'</td></tr>';
	else $tx.='<tr id="tr'.$j.'_'.$i.'" onclick="f_c('.$j.','.$i.')"><td>'.$pos1.'</td><td>'.((substr($pos2,0,1)>'0')?'a':'').'</td>'.(($j<8)?('<td>'.((substr($pos2,1,1)>'0')?'u':'').'</td><td>'.$tm.'</td>'):'').'<td id="d3_'.$j.'_'.$i.'">'.htmlspecialchars($pos3).'</td></tr>';
