...
function f_postPred() {
	...
	for (i=1; i<351; i+=1) {
		if($.id('i'+i+'_3')) {
			for (j=3; j<9; j+=1) { if($.id('i'+i+'_'+j)) s+=f_replace($.id('i'+i+'_'+j).value,1)+t; }
			for (j=9; j<16; j+=1) { if($.id('src'+i+'_'+j)) s+=f_replace($.id('src'+i+'_'+j).innerHTML,1)+(j<15?t:''); }
		} else s+=' ';
		s+=n;
	}
	f_show_query(); v_mode=1; $.httpPost($.id('iddomain').innerHTML+'/cssjs/price_js_02.php','mfd',['data',s,'hash',v_hash],v_XHR); v_interval=setInterval(function(){f_hRSC()},100);
}
...
function f_priceCalc() {
	var i, k, s, z, c, p=$.id('iPKurs'); if (!p) return;
	k=parseFloat(f_replace(p.value,1).replace(/\,/gm,'.'))||1; if(k<0)k=0-k; p.value=k.toFixed(2); k=parseFloat(k.toFixed(2));
	for (i=1; i<351; i+=1) {
		if ($.id('i'+i+'_8')) {
		  p=(parseInt($.id('i'+i+'_8').value,10)||0)*k; console.log('k='+k+' p='+p+'  i'+i+'_8');
		  $.id('i'+i+'_7').value=p.toFixed(0);
		  if (1==1 && (i==3 || i==58)) {
			$.id('i'+i+'_5').value=p.toFixed(0);
			$.id('i'+i+'_4').value=p.toFixed(0);
			$.id('i'+i+'_3').value=p.toFixed(0);
		  } else {
			$.id('i'+i+'_5').value=(p*1.05).toFixed(0);
			$.id('i'+i+'_4').value=(p*1.10).toFixed(0);
			$.id('i'+i+'_3').value=(p*1.15).toFixed(0);
		  }
		}
	}
}
/*
function f_priceCalc() {
	var i, k, s, z, p=$.id('iPriceK'), c=$.id('iPriceA'); if (!p||!c) return;
	k=parseFloat(f_replace(p.value,1).replace(/\,/gm,'.'))||1; if(k<0)k=0-k; p.value=k.toFixed(2); k=parseFloat(k.toFixed(2));
		...
		if ($.id('d'+i+'_0')) {
			for(j=3; j<9; j+=1) {
				if ($.id('i'+i+'_'+j) && $.id('src'+i+'_'+j)) {
					c=$.id('src'+i+'_'+j).innerHTML; z=(c.indexOf('*')==-1)?'р.':'р.*';
