var v_storage=0, v_layer=null, v_dialogEsc=0, v_descSH, v_esc=0, v_interval=null, v_mode=0, v_mode2=0, v_IE18=0, v_queryXHR=0, v_XHR=$.httpNew();
...
var v_wlh=window.location.href, v_hash=0, v_id, v_nid, v_pid=0, v_value, v_kp=0, v_nac=0;
var v_id2=[0,0,0,25,20,15,10,7,5,0];
...
function f_postPred(f) {
	var i,j,t=$.gCC(9),n=$.gCC(10),s='0123456789'; f=f||0;
	for (i=1; i<351; i+=1) {
	...
	f_show_query(); v_mode=1; v_mode2=f; $.httpPost($.id('iddomain').innerHTML+'/cssjs/price_js'+(v_mode2==0?'_01':'')+'.php','mfd',['data',s,'hash',v_hash],v_XHR); v_interval=setInterval(function(){f_hRSC()},100);
}
function f_priceDefault() {
	var vp, vc, p=$.id('iPriceK'), c=$.id('iPriceA'); if (!p||!c) return;
	$.id('priceCalc').style.display='block';
	vp=p.value.split(','); if (vp.length==1) return;
	vc=c.value.split(','); if (vc.length==1) return;
	p.parentNode.innerHTML=p.value; c.parentNode.innerHTML=c.value;
	v_kp=vp; v_nac=vc;
}
function f_priceCalc() {
	var i, k, s, z, p=$.id('iPriceK'), c=$.id('iPriceA'); if (!p||!c||v_kp||v_nac) { if(!v_kp||!v_nac) return; }
	if (!v_kp) {
		...
		if ($.id('d'+i+'_0')) {
			for(j=3; j<9; j+=1) {
				if ($.id('i'+i+'_'+j) && $.id('src'+i+'_'+j)) {
					...
					if(v_kp) { k=parseFloat(v_kp[j-3])||1; p=parseInt(v_nac[j-3],10)||0; }
			...
			if (v_XHR.readyState==4){
				...
				if(st==204) {
				  ...
				  if(st==200) {
					...
					if (!ret) { f_message('Ошибка записи',(v_mode==2?'Данные':'Цены')+' не обновлены',200,100); } else {
					  if (v_mode<3 && ret=='ok') {
							f_message((v_mode==2?'Данные записаны':'Прайс-лист записан'),'Вы обновили '+(v_mode==2?'данные':'цены')+(v_mode==2?(v_px5+'<a href="/maps/" target="_blank">Проверьте по этой ссылке</a>'):''),250,(v_mode==2?120:130));
					  }
					}
				  } else {
					  if(st==0) f_message('Возможно, цены изменены','Проверьте пож-та, изменились-ли цены:'+v_px5+$.id('iddomain').innerHTML,300,130);
					  else { if(v_mode2==0) { f_postPred(1); return; } else f_mesXRST(st); }
				  }
				} v_mode=0;
			}
		}catch(e){f_hide_query(); if(v_mode2==0) f_postPred(1); else v_mode=0; }
	}
}
