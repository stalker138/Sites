...
include "vpb_config.php"; fpb_authorization(4);
...
$file='<!DOCTYPE html><html><head><meta http-equiv="Content-Type" content="text/html; charset=UTF-8"><meta name="robots" content="noindex, nofollow"><title>Vcorp generator - The structure of nested elements</title>';
$file.='<link rel="stylesheet" href="vpb.css" type="text/css"/><script type="text/javascript" charset="UTF-8" src="vpb.js"></script></head><body><br><div style="position:relative; width:1000px; margin:0 auto; padding:10px; border:solid 1px #000080; border-radius:10px;">';
...
$file.='<p style="float:left;padding-left:100px;text-align:center;">The structure of nested elements.</p><br>&nbsp;';
$out=$file.'<menu id="nestedMenu">'.$out.'</menu><br></div></body></html>';
	...
	if ($pos==$pos2) {
		if ($l>8) $li=$liR.htmlspecialchars(substr($t,$pos,8)).'…*}</span></li>'; else {
			if ($li=='{*tpl0*}') {
				...
				if (substr($pos1,-4)=='.php' || substr($pos1,-4)=='.htm' || substr($pos1,-5)=='.html') $li='<li><div class="pb_m5"></div><span class="pb_ms5">'.$li.'</span> &nbsp;&nbsp;<span style="font-weight:normal;">Служебный шаблон - Подключение title, keywords, description</span></li>';
				else $li='<li><div class="pb_m4"></div><span style="color:#800000;font-weight:bold">'.$li.'</span> <span style="color:#800000">Служебный шаблон - Подключение title, keywords, description в файл : </span><span style="color:#800000;font-weight:bold">'.htmlspecialchars($pos1).'</span></li>';
			} else {
				$n=(int)substr($li,5,3); $pos1=strpos($tx[6],chr(10).'*number*'.$n.chr(10),0); if ($n==0 || $pos1==false) $li=$liR.$li.'</span></li>'; else {
					...
					$li = ($txt=='') ? ('<li><div class="pb_m5"></div><span class="pb_ms5">'.$li.'</span>'.$pos1.'</li>') : ('<li class="pb_mc"><div class="pb_m2"></div><span class="pb_ms2"'.((strpos($txt,$c)==false)?'':$c).'>'.$li.'</span>'.$pos1.'<menu>'.$txt.'</menu></li>');
				}
			}
		}
	}
