...
for ($j=0; $j<6; $j++) {
  ...
  for ($i=1; $i<1000; $i++) {
	...
	for ($ii=0; $ii<count($pba_subst); $ii++) {
		$v_title = str_replace('{'.$ii.'}', $pba_subst[$ii], $v_title);
		$v_desc = str_replace('{'.$ii.'}', $pba_subst[$ii], $v_desc);
		$v_keyw = str_replace('{'.$ii.'}', $pba_subst[$ii], $v_keyw);
	}
	...
	$pos2=strpos($t,'{*tpl0*}',0); if ($pos2!=false) { // Если вставлять title,keywords,description необходимо ТОЛЬКО php-файлам, то раскомментировать 1 и 3 строки ниже.
	  // $file=$v_link; if (substr($file,-1)=='/') $file.=$pb_main_file; if (substr($file,-4)=='.php') {
		$t=substr($t,0,$pos2).'<meta http-equiv="Content-Type" content="text/html; charset=UTF-8"/><title>'.$v_title.'</title><meta name="keywords" content="'.$v_keyw.'"/><meta name="description" content="'.$v_desc.'"/>'.substr($t,$pos2+8);
	  // }
	...
	$pos=0; while (true) {
		...
		if ($pb_search_url) { // Проверка существующих внутренних URL
			...
			if ($link!=$pb_empty_url && (substr($link,0,1)!='#') && (substr($link,0,10)!='javascript') && (substr($link,0,7)!='mailto:') && (substr($link,0,6)!='skype:') && (substr($link,0,4)!='tel:') && (substr($link,0,8)!='viber://') && (substr($link,0,7)!='http://') && (substr($link,0,8)!='https://')) for ($k=0; $k<6; $k++) { $pos1=strpos($tx[$k],('*link*'.$t3.$pos3.chr(10)),0); if($pos1>0) break; }
