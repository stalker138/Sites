...
$v_options = ($pb_autocheck?'1':'0').(($v_index<6) ? (($pb_titles?'1':'0').($pb_sitemap?(($v_index>0||($pb_sitemap_start>0&&$pb_sitemap_start<=$number))?'5':'0'):'0').'0000000') : '000000000');
...
if ($v_mode==21) {
	$v_options=trim(fpb_post('options')); $v_options=($v_index<6) ? $v_options : (substr($v_options,0,1).'000000000'); $v_name=trim(fpb_post('name')); $v_link=trim(fpb_post('link')); $v_title=trim(fpb_post('title')); $v_keyw=trim(fpb_post('keyw')); $v_desc=trim(fpb_post('desc'));
	if ($v_index<6) {
	...
	if (substr($v_options,0,1)!='0') { // Авто - проверка исходного кода (auto-check)
		...
		$pos=0; while (true) {
			...
			if ($pb_search_url) { // Проверка существующих внутренних URL
				...
				if ($link!='empty' && (substr($link,0,1)!='#') && (substr($link,0,10)!='javascript') && (substr($link,0,7)!='mailto:') && (substr($link,0,6)!='skype:') && (substr($link,0,4)!='tel:') && (substr($link,0,7)!='http://') && (substr($link,0,8)!='https://')) for ($i=0; $i<6; $i++) { $pos1=strpos($tx[$i],('*link*'.$t3.$pos3.chr(10)),0); if($pos1>0) break; }
